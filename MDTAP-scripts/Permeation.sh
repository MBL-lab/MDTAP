...
Recommended dimensions (Xmax Xmin Ymax Ymin Zmax Zmin) for your channel are:" $a $a1 $b $b1 $c $c1
...
spin()
{
    ...
    while [ $t -eq 1 ]; do
        for (( s=0; s<${#load}; s++ )); do
            sleep 0.75
            echo -en "Calculating the permeation of $molecule... ${load:$s:1}" "\r"
        done
    done
}
spin &
...
echo "Based on the frequency of occurrence of the atoms ($molecule) in the PDBs as shown above, enter the minimum frequency that you would like to consider for the calculation and press [ENTER]:
(e.g. if you enter 3, all the atoms ($molecule) that occurred a minimum of 3 times within the defined limits will be taken for the calculation)"
    read maxscore
if [ -z $maxscore ]; then
...
score=0
maxzdim=$[${zhh}-${zll}]
echo "The height of the channel (Z-axis) as per the Z-coordinates defined by you (Zmax=$Zmax and Zmin=$Zmin) is ${maxzdim#-}"
...
for ((i=0; i<${#maxatom[@]}; i=i+1)); do
    ...
    cat $address/$filename/temp.pdb | awk -F ':' '{print $1}' | awk -F '.pdb' '{print $1}' | awk -F 'step_' '{print $2}' | awk '{ for (j = 1; j <= NF; j++) {
        lin[i++] = $j;
        }
    }
    END {
    ...
    for ((ii=0 ; ii<${#perm[@]}; ii=ii+1)); do
        ...
        tempEnZ1=$[${tempEnZ0}-${zp}]
        tempEnZ2=$[${tempEnZ0}-${zn}]

        tempExZ0=`printf %0.f $exitZ`
        tempExZ1=$[${tempExZ0}-${zp}]
        tempExZ2=$[${tempExZ0}-${zn}]
        ...
        if ( [ ${entryPDB} == ${enPDB} ] || [ ${exitPDB} == ${exPDB} ] ); then
            if ( [ $tempEnZ0 -lt $zp ] || [ $tempExZ0 -gt $zn ] ); then    # if the molecule is already inside the channel
...
rm -r $address/$filename/step.pdb $address/$filename/tempPDBs
endtime=`date`
...
echo "You can verify the PDBs generated by this module and further track the path traced by the atom numbers listed using the 'Track molecule' (APM #8)"
