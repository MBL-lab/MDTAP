...
echo "This file contains the list of channel residues that are in close proximity (< 3.5 Å) to the molecule(s) of interest ($resname) in each DCD frame.
Column 1: DCD frame number, Column 2: Residue name, Column 3: Residue number" > $address/$filename/Residues.dat
awk '/DCD Frame:/ {split($0, a, ": "); frame=a[2]; next} /^[0-9]/ {print frame, $8, $9}' $address/$filename/Distance.dat | sort -u | tr -d ',' >> $address/$filename/Residues.dat
echo "This file contains the frequency of the channel residues that are in close proximity (< 3.5 Å) to the molecule(s) of interest ($resname) across all DCD frames.
