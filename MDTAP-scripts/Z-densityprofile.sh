...
echo " ----------------------------------------------------------------------------------"
echo "This module calculates the number of molecules present along the protein axis (Z-axis). The user-defined space is divided into slices of 1Å thickness, and the number of molecules in each slice is calculated." | fold -sw 80 | sed "s/^/  /g"
echo "===================================================================================="
echo ""
...
if [ -z $chainid ]; then
    ...
    b=`echo "${y[*]}" | sort -nr | head -n1`
    b1=`echo "${y[*]}" | sort -nr | tail -n1`
    awk '{print $9}' $address/$filename/step.pdb > $address/$filename/z.txt
...
spin()
{
    ...
    while [ $t -eq 1 ]; do
        for (( s=0; s<${#load}; s++ )); do
            sleep 0.75
            echo -en "Calculating the Z-density profile... ${load:$s:1}" "\r"
        done
    done
}
spin &
