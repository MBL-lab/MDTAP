...
{
    echo -e "\nSince the given membrane channel is unidirectional (i.e. either influx/efflux is zero), the diffusion permeability coefficient will be calculated only for one direction."
    d=`echo "2.98*${q1}" | bc -l`
}
else
{
    echo -e "\nSince the given membrane channel is bidirectional (i.e. both influx and efflux are greater than zero), the diffusion permeability coefficient will be calculated for both directions."
