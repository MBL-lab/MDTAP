...
while [ ${start} -le  ${end} ]; do
    awk -v xpp="${xp}" -v xnn="${xn}" -v ypp="${yp}" -v ynn="${yn}" -v zpp="${zhh}" -v znn="${zll}" '{if ( $3<=xpp && $3>=xnn && $4<=ypp && $4>=ynn && $5<=zpp && $5>=znn ) printf " %5i \n", $1}' "${address}"/"${filename}"/coord_${start}.dat >> $address/$filename/all-atoms.dat
    start=$[${start}+${skip}]
done

start=`echo ${start1}`

echo "This file contains the list of atom numbers of $atname that were found within the channel limits (Xmax=$xp, Xmin=$xn, Ymax=$yp, Ymin=$yn, Zmax=$zp, and Zmin=$zn)." > $address/$filename/Atom_list.dat

sort -u $address/$filename/all-atoms.dat >> $address/$filename/Atom_list.dat

name=( $(awk '{print $1}' ./$address/$filename/Atom_list.dat) )  # unique atom array
x=`echo ${#name[@]}`  # number of unique atoms to be used in for loop for score calculation
score=0
echo "This file contains the frequency of occurrence of the permeating molecule: $atname
Column 1: atom numbers of $atname in the user-defined area     Column 2: Permeation frequency" > $address/$filename/Atom_frequency.dat
...
echo "Based on the frequency of occurrence of the atoms ($atname) in the DCD frames as shown above, enter the minimum frequency that should be considered for the calculation and press [ENTER]:
(e.g. if the entered value is 3, all the atoms ($atname) that occurred a minimum of 3 times within the defined limits will be taken for the calculation)"
    read maxscore
if [ -z $maxscore ]; then
...
echo "This file contains the list of atom numbers of $atname that have a occurred at least $maxscore times within the user-defined limits." >> $address/$filename/Permeation_max.dat
...
------------------------------------------------------------------------------
      Molecule of interest = ${atname}
      Start DCD file = ${stdcd}
      End DCD file = ${endcd}
      Start DCD frame = ${start}
      End DCD frame = ${end}
      DCD frames to skip = ${skip}
      Seg ID = ${segid}
...
for ((pdb=${start}; pdb<=${end}; pdb=pdb+${skip})); do
    cp $address/$filename/coord_${pdb}.dat $address/$filename/tempPDBs/coord_${pdb}.dat
done
...
for ((i=0; i<${#maxatom[@]}; i=i+1)); do
    ...
    grep "${maxatom[$i]}" $address/$filename/step_1.pdb > $address/$filename/file1.txt
    grep "${maxatom[$i]}" $address/$filename/tempPDBs/coord_*.dat | awk '{ $1=""; print $0 }' | awk  -v xpp="${xp}" -v xnn="${xn}" -v ypp="${yp}" -v ynn="${yn}" -v zpp="${zp}" -v znn="${zn}" '{if ( $3<=xpp && $3>=xnn && $4<=ypp && $4>=ynn && $5<=zpp && $5>=znn ) print $3, $4, $5}' | sort -Vk1 > $address/$filename/file2.txt
    awk 'NR==1 {line1=sprintf("%-6s%-6s%-5s%-5s%4d", $1, $2, $3, $4, $5); suffix=sprintf("%22s", $9); next} {printf "%s %8.3f %8.3f %8.3f %s\n", line1, $1, $2, $3, suffix}' $address/$filename/file1.txt $address/$filename/file2.txt >> $address/$filename/${maxatom[$i]}.pdb
    echo "END" >> $address/$filename/${maxatom[$i]}.pdb
    grep "${maxatom[$i]}" $address/$filename/tempPDBs/coord_*.dat | awk  -v xpp="${xp}" -v xnn="${xn}" -v ypp="${yp}" -v ynn="${yn}" -v zpp="${zp}" -v znn="${zn}" '{if ( $4<=xpp && $4>=xnn && $5<=ypp && $5>=ynn && $6<=zpp && $6>=znn ) print $1}' > $address/$filename/temp.pdb
    cat $address/$filename/temp.pdb | sort -V | awk -F 'coord_' '{print $2}' | awk -F '.dat' '{print $1}' | awk -v skip=$skip '{ for (j = 1; j <= NF; j++) {
        lin[i++] = $j;
        }
    }
...
END {
    ...
    echo "================================================================================"
    echo -e "   ATOM number = "${maxatom[$i]}"\n--------------------------------------------------------------------------------\n   Number of permeating events = "${#perm[@]}"\n   DCD frame ranges for permeation = "${perm[@]}"\n" 
  
    for ((ii=0 ; ii<${#perm[@]}; ii=ii+1)); do
        enPDB=`echo ${perm[$ii]} | cut -d"-" -f1 | tr -d ' '`  # subtracted by 1 to find next DCD frame; required for channel dimension
        exPDB=`echo ${perm[$ii]} | cut -d"-" -f2 | tr -d ' '`  # added by 1 to find previous DCD frame; required for channel dimension
        entryZ=`awk -v atm=${maxatom[i]} '{if ( $1 == atm ) print $5}' $address/$filename/coord_$enPDB.dat` # Z-coordinate of the exiting DCD frame
        exitZ=`awk -v atm=${maxatom[i]} '{if ( $1 == atm ) print $5}' $address/$filename/coord_$exPDB.dat` # Z-coordinate of the exiting DCD frame
        ...
        entryZ1=`awk -v atm=${maxatom[i]} '{if ( $1 == atm ) print $5}' $address/$filename/coord_$entryPDB.dat` # Z-coordinate of previous DCD frame; required for directionality
        exitZ1=`awk -v atm=${maxatom[i]} '{if ( $1 == atm ) print $5}' $address/$filename/coord_$exitPDB.dat` # Z-coordinate of exiting DCD frame; required for directionality 
        ...
        echo -e "     Entry frame = $enPDB \n     Exit frame  = $exPDB \n     Entry Z-coordinate = $entryZ \n     Exit Z-coordinate  = $exitZ \n\n   Using the Z-coordinates of DCD frames before and after entry & exit respectively:\n     Current dimension = ${actdim#-} \n     Channel dimension = ${maxzdim#-}\n"
...
rm -rf $address/$filename/step*.pdb $address/$filename/tempPDBs $address/$filename/coord*.dat 2> /dev/null
rm $address/$filename/file1.txt $address/$filename/file2.txt $address/$filename/atmlist.dat 2> /dev/null
endtime=`date`
